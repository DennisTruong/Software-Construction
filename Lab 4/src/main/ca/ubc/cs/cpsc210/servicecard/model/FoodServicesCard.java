...
public class FoodServicesCard {
    ...
    public boolean makePurchase(int amount) {
        if (this.balance >= amount) {
            this.balance = this.balance - amount;
            this.rewardPoints = this.rewardPoints + (amount * REWARD_POINTS_PER_CENT_CHARGED);
            while (this.rewardPoints >= POINTS_NEEDED_FOR_CASH_BACK) {
                this.balance = this.balance + CASH_BACK_REWARD;
                this.rewardPoints = this.rewardPoints - POINTS_NEEDED_FOR_CASH_BACK;
            }
            return true;
        } else {
            return false;
